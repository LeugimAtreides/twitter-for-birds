...
const ProviderCardForResults: React.FC<ProviderProps> = ({
  provider,
  ...
  showRequest = false,
  ratingOnClick = () => false,
  blockName = "deaui-ProviderProfile",
  theme,
}) => {
  ...
  const onRequestAppointment = () => {
    ...
    history.push(`/find-care/scheduling/request/location/${officeId}/physician/${providerId}`);
    window.scrollTo(0, 0);
  };
        ...
        <div className={`${blockName}__Provider-Stats`}>
          ...
          {showRating && provider.hasAverageRating &&
            <div role="button" tabIndex={0} onClick={ratingOnClick} className={`${blockName}__Rating`}>
              <Rating blockName={`${blockName}__Rating--stars`} providerRating={Number(provider.averageRating?.toFixed(1))} />
          ...
          {showNewPatientFlag && <NewPatientFlag blockName={`${blockName}__Provider-Stats--appointments`} newPatients={provider["availability.accepting_new_patients"]} />}
...
export default ProviderCardForResults;
